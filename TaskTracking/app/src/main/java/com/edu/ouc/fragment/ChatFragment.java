            ...
            public void run() {
                ...
                try {
                    ...
                    if(netWorkUtils.isNetworkConnected(getContext())==false){ //若网络未连接
                        ...
                        }else if (role.equals("班长")) { //查询属于本单位且未确认的订单，以及查询实时类型任务的订单
                        ...
                        }else if (role.equals("职员")) { //查询属于本单位且未确认的订单，以及查询实时类型任务的订单
                            SelectDataFromServer selectDataFromServer=new SelectDataFromServer("http://10.0.2.2:8080/TaskTrackingService/getYuanGongWaitTaskInfo.do?sql=where+tc.confirm_unit+like+'%,3:0,%'");
                            if(selectDataFromServer.getContent().equals("error")){
                                handler.sendEmptyMessage(0);//发送消息到handler，提示出错了
                            }else{
                                JSONObject jsonObject=new JSONObject(selectDataFromServer.getContent());
